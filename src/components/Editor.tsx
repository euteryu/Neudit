...
const MenuBar = ({ editor }) => {
  ...
  return (
    <div className="sticky top-0 z-10 flex flex-wrap gap-2 p-2 mb-4 rounded-xl bg-neu-base/90 backdrop-blur-sm border-b border-neu-dark/10 transition-all">
      ...
      <div className="flex gap-2">
        <NeuButton onClick={() => editor.chain().focus().undo().run()} className="!p-2 w-10 h-10">
          <Undo size={18} />
        </NeuButton>
        <NeuButton onClick={() => editor.chain().focus().redo().run()} className="!p-2 w-10 h-10">
          <Redo size={18} />
        </NeuButton>
      </div>
    </div>
...
export const Editor = ({ content, onChange }) => {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        // Tailwind Typography (prose) classes for nice styling
